  ...
  <xsl:for-each select="trd">
    <trd lng="{$lng}">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:apply-templates/> <!-- tio krom teksto ankaŭ transprenas trd-internajn ind, pr, klr, baz -->
        </xsl:otherwise>
      </xsl:choose>
    </trd>
  </xsl:for-each>
...
<xsl:template match="trd[@lng]">
  <xsl:copy>
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:apply-templates select="@lng|text()|ind|pr|klr[@tip='ind' or @tip='amb']|baz"/>
        </xsl:otherwise>
      </xsl:choose>
  </xsl:copy>
</xsl:template>
...
<xsl:template match="kap|ofc|var|@mrk|@lng|uzo[@tip='fak']|mlg
  |ind|pr|klr[@tip='ind' or @tip='amb']|baz">
