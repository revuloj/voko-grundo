...
sub desc_mlg {
    my $d = shift;
    $d =~ s/^(мін|рел|арх|геогр|геол|жив|фізіол|вульґ|наук|політ|фарм|хiм|фарм|спорт|іст|іхт|муз|друк|тех|мед|бот|мор|бібл|лінґв|хім|театр|ел|текст|фізіол|анат|зоол|астр|кул|міф|мист|орн|фісоф|філос|біол|хем|прям|бакт|фіз|мор|зал|літ|інформ|мат)\.\s*//g;
    $d =~ s/^v[nt]\s*//g;
